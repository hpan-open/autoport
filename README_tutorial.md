...
* It may seem like a time-consuming process, to develop an initial
  ports selection and a corresponding set of configuration options,
  such as may serve to suit to any specific set of individual usage
  requirements.
  
  To an extent, the time-consuming nature of the port build process
  -- as in regards to diagnosing of port builds, whether for purposes
  of distribution design, release engineering, or issue tracking --
  may be  mitigated with the extent of log data produced by
  Poudriere.

  Individual build  failures can be easily diagnosed with review of
  log files generated  by Poudriere. Build failures can then be
  addressed with individual port reconfigurations, port updates, or
  in-depth issue tracking onto individual upstream codebases, as
  published of the ports tree and distfile archives.
      
* Some configuration options -- for instance, the VirtualBox `MANUAL`
  option -- when selected, may result in adding a substantial number
  of build dependencies to a port, thus increasing the total build
  time and media "Footprint" of the build. New build dependencies may
  be introduced as directly by way of immediate dependencies and
  indirectly, by way of dependencies introduced with immediate
  dependencies. Some of a port's dependencies -- in a context of build
  dependencies and/or runtime dependencies -- may be introduced to a
  port as contingent on selection of specific configuraiton options.
  
  Port selections may be carefully reviewed, as in regards to
  individual configuration options, effects demonstrated at build 
  time, and results demonstrated at runtime within individual,
  configured _build sets_.
